...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-slate-600">Manage employee accounts, roles, and personal details.</p>
        </div>
        ...
        <div class="p-4 border-b border-slate-100 bg-slate-50/50">
            <form action="{{ route('admin.users.index') }}" method="GET" class="flex items-center gap-3">
                <div class="relative flex-1 max-w-md">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search employees..." class="block w-full pl-10 pr-3 py-2 border border-slate-200 rounded-full text-sm focus:ring-emerald-500 focus:border-emerald-500">
                </div>
            ...
            <table class="min-w-full table-auto w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        ...
                        <th class="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Roles</th>
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                         ...
                         <td class="px-6 py-4 whitespace-nowrap text-sm text-right space-x-3">
                             <a href="{{ route('admin.users.edit', $user->id) }}" class="text-emerald-600 hover:text-emerald-800 font-semibold transition-colors">Edit</a>
                             @if(auth()->user()->hasRole('administrator'))
                                 <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" class="inline">
                                     @csrf
                                     @method('DELETE')
                                     <button type="submit" class="text-rose-600 hover:text-rose-800 font-semibold transition-colors" onclick="return confirm('Are you sure?')">Delete</button>
                                 </form>
                             @endif
                         </td>
                    </tr>
